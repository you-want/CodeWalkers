...
本项目基于强大的 **Tauri** 架构，使用 **Rust** 编写高性能、低资源占用的后端逻辑，使用 **React + TypeScript** 构建精美的透明前端界面。它是原版 `Lil Agents` 概念的全面现代化、跨平台复刻与升级版。

### ✨ 核心特性 (Key Features)

- **🏃‍♂️ 桌面虚拟陪伴**：角色会在屏幕下方自由漫游，有真实的行走动画与休息状态。
- **🖱️ 像素级点击穿透**：采用高精度 Canvas Alpha 检测技术。点击角色实体时可以拖拽，点击角色旁边的透明区域时，鼠标事件会完美穿透到你的桌面或后面的软件。
- **🖥️ 沉浸式 AI 终端**：内置交互终端，你可以直接在应用内发送消息，并得到实时反馈。
- **🎵 原生音效反馈**：发送消息、接收回复以及角色巡游到达终点时，都会有清脆的提示音。
- **🎨 多角色与主题系统**：
  - 支持一键切换角色（Bruce / Jazz）。
  - 支持三种不同的终端主题风格：`Playful` (默认)、`Hacker` (极客黑绿)、`Monochrome` (复古黑白)。
...
如果你想打包一个可独立运行的 App 给别人使用：

```bash
pnpm tauri build
```
编译产物将会生成在 `src-tauri/target/release/bundle` 目录下（如 macOS 的 `.app` 文件或 `.dmg` 安装包）。
...
2. **呼出/隐藏终端**：小人旁边会一直跟随一个终端面板，你可以点击右侧面板进行输入。
...
- `src/` - React 前端代码 (UI, IPC 交互, 拖拽逻辑, 动画渲染)
- `src-tauri/` - Rust 后端代码 (窗口管理, 进程挂起, 透明度控制, 系统托盘)
